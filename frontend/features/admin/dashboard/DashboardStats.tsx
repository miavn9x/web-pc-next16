...
import { productService } from "../components/quan-ly-san-pham/services/productService";
import { postService } from "../components/quan-ly-noi-dung/services/postService";
import { couponService } from "../components/quan-ly-ma-giam-gia/services/servicesCoupons";
...
const DashboardStats = () => {
  ...
  const [revenueInPeriod, setRevenueInPeriod] = useState<{
    vi: number;
    ja: number;
  }>({ vi: 0, ja: 0 });
  ...
  const initDashboard = async () => {
    ...
    try {
      ...
      // 2. Fetch Products
      const productsRes = await productService.getAll({ page: 1, limit: 1 });
  ...
  const processData = (
    ...
    const rev = filteredOrders.reduce(
      (acc, order) => {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          params.forEach((param: any) => {
            ...
            const val = new Intl.NumberFormat(localeCode, {
              style: "currency",
              currency,
            }).format(param.value);
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case "CONFIRMED":
        return t.options.confirmed;
      case "COMPLETE":
        return t.options.complete;
      case "CANCELLED":
        return t.options.cancelled;
      default:
        return status;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="mt-4 flex gap-3">
                    {section.details.map((detail, idx) => (
                      <span
                        key={idx}
                        className="text-xs font-medium px-2.5 py-1 bg-gray-50 text-gray-600 rounded-full border border-gray-200"
                      >
                        {detail}
                      </span>
                    ))}
        ...
        <div
          id="section-recent-orders"
          className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 h-fit"
        >
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-gray-900">
              {t.recentOrders} {getRangeLabel().toLowerCase()}
            </h3>
          </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      ...
                      ).toLocaleDateString(locale === "vi" ? "vi-VN" : "ja-JP")}
                    </p>
