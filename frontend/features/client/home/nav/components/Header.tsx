    ...
    <div className="bg-white lg:bg-[#103E8F] text-gray-800 lg:text-white py-2 lg:py-4 shadow-sm lg:shadow-lg z-40 sticky top-0 transition-colors duration-300">
        <div className="container mx-auto px-4 flex items-center justify-between gap-2 lg:gap-4 relative">
          ...
          <Link href="/" className="flex items-center gap-2.5 group shrink-0">
            ...
            <span className="text-lg md:text-2xl font-bold tracking-tight hidden sm:block lg:text-white text-[#103E8F]">ComputerWorld</span>
          </Link>
          ...
          <div className="flex items-center gap-2 md:gap-4 lg:gap-6 shrink-0">
            ...
            <div className="flex items-center gap-1 lg:gap-3 cursor-pointer lg:hover:bg-white/10 px-1 lg:px-2 py-1.5 rounded-md transition-colors duration-200 relative group">
              <div className="relative">
                <ShoppingCart className="w-6 h-6 lg:w-[26px] lg:h-[26px] text-gray-700 lg:text-white" strokeWidth={1.5} />
                <span className="absolute -top-1.5 -right-1.5 bg-[#FF0000] lg:bg-[#FFB800] text-white lg:text-[#103E8F] text-[10px] w-4 h-4 flex items-center justify-center rounded-full font-bold shadow-sm group-hover:scale-110 transition-transform">0</span>
              </div>
              <div className="hidden lg:flex flex-col leading-tight">
                 <span className="text-[11px] opacity-80">Giỏ hàng</span>
                 <span className="text-[13px] font-semibold">0đ</span>
              </div>
            </div>

             {/* Mobile/Tablet Menu Toggle (New Design) */}
            <button type="button" onClick={onToggleMobileMenu} className="lg:hidden flex flex-col items-center gap-0.5 text-gray-700 p-1">
                {isMobileMenuOpen ? <Menu className="w-6 h-6 md:w-7 md:h-7 text-red-600" /> : <Menu className="w-6 h-6 md:w-7 md:h-7" />}
